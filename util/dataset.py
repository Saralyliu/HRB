...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None,mode='train'):
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        # #filter images containing objects of novel categories during meta-training
        # if mode == 'train':
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, transform=None, mode='train', use_coco=False, use_split_coco=False):
        ...
        if self.mode == 'train':
            self.data_list, self.sub_class_file_list = make_dataset(split, data_root, data_list, self.sub_list,self.mode)
            assert len(self.sub_class_file_list.keys()) == len(self.sub_list)
        elif self.mode == 'val':
            self.data_list, self.sub_class_file_list = make_dataset(split, data_root, data_list, self.sub_val_list,self.mode)
