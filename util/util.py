...
def compute_prior_mask1(final_supp_list, mask_list, corr_query_mask_list, query_feat, cosine_eps):
    for i, tmp_supp_feat in enumerate(final_supp_list):
        resize_size = tmp_supp_feat.size(2)
        tmp_mask = F.interpolate(mask_list[i], size=(resize_size, resize_size), mode='bilinear', align_corners=True)

        tmp_supp_feat = Weighted_GAP(tmp_supp_feat, tmp_mask)
        q = query_feat
        s = tmp_supp_feat
        ...
        corr_query = F.interpolate(corr_query, size=(query_feat.size()[2], query_feat.size()[3]), mode='bilinear',
                                   align_corners=True)
...
def compute_prior_mask(final_supp_list, mask_list,  corr_query_mask_list, query_feat, cosine_eps):
    for i, tmp_supp_feat in enumerate(final_supp_list):
        resize_size = tmp_supp_feat.size(2)
        resize_size = int(resize_size*0.5)
        tmp_supp_feat = F.interpolate(tmp_supp_feat, size=(resize_size, resize_size), mode='bilinear', align_corners=True)
        tmp_mask = F.interpolate(mask_list[i], size=(resize_size, resize_size), mode='bilinear', align_corners=True)

        tmp_supp_feat = tmp_supp_feat * tmp_mask
        q = query_feat
        q = F.interpolate(q, size=(resize_size, resize_size), mode='bilinear',
                                      align_corners=True)
        s = tmp_supp_feat
        bsize, ch_sz, sp_sz, _ = q.size()[:]
...
def compute_prior_mask_bdc(final_supp_list, mask_list, corr_query_mask_list, query_feat, cosine_eps):
    for i, tmp_supp_feat in enumerate(final_supp_list):
        resize_size = tmp_supp_feat.size(2)
        tmp_mask = F.interpolate(mask_list[i], size=(resize_size, resize_size), mode='bilinear', align_corners=True)
        ...
        tmp_query = q
        tmp_query = tmp_query.contiguous().view(bsize, ch_sz, -1).permute(0, 2, 1)
        tmp_query = BDCovpool(tmp_query)

        tmp_supp = s
        tmp_supp = tmp_supp.contiguous().view(bsize, ch_sz, -1).permute(0, 2, 1)
        tmp_supp = BDCovpool(tmp_supp)
        tmp_supp = tmp_supp.contiguous().permute(0, 2, 1)
